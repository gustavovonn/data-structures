...
template<typename T>
int BinaryTree<T>::height(Node* node, int current_height) {
/*
...
*/
    height = current_height;  // Father height

    if (node->right == nullptr && node->left == nullptr) {  // Se for a folha
        return height++;
    }

    if(node->left != nullptr) {
        left_height = node->height(node->left, current_high++);
    }

    if (node->right != nullptr) {
        right_height = node->height(node->right, current_high++);
    }
