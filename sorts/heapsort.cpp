...
void heapify(int* array, int size, int i) {
    int maior_index = i;  // Inicia com o indice do nodo atual
    int esquerda_index = (2 * i) + 1;  // Calculo do indice do filho da esquerda
    int direita_index = (2 * i) + 2;  // Calculo do indice do filho da direita


    /* Verifica se esquerda_index existe. Caso sim, se ele for maior, 
    substitui o valor de maior_indice pelo indice da esquerda
    */
    if (esquerda_index < size && array[esquerda_index] > array[maior_index])
        maior_index = esquerda_index;


    /* Verifica se direita_index existe. Caso sim, se ele for maior, 
    substitui o valor de maior_indice pelo indice da direita
    */
    if (direita_index < size && array[direita_index] > array[maior_index])
        maior_index = direita_index;
    

    /*Caso um dos filhos verificados tenha o valor de chave maior que
    o do pai, os valores sao trocados entre eles, ajustando a heap caso
    ela estivesse com propriedade heap maxima perdida.*/
    if (maior_index != i) {
        int temp = array[i];
        array[i] = array[maior_index];
        array[maior_index] = temp;
    
        heapify(array, size, maior_index);  // Recursivamente verifica as subárvores
    }
}

void heapsort(int* array, int size) {

    // Construindo a heap (prioridade de maior valor - heap maxima)
    for (int i = size / (2 - 1); i >= 0; i--) {
        heapify(array, size, i);
    }

    for (int i = size - 1; i > 0; i--) {
        /* Colocando as raizes para a esquerda no vetor: basicamente,
        ...
        */
        ...
        // Vai reajustando a heap maxima local para manter a coerencia
        heapify(array, i, 0);
    }
}
